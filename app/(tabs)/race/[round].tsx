...
export default function RaceDetailScreen() {
  ...
  function ResultTable({ results, type }: { results: any[]; type: 'race' | 'quali' | 'sprint' }) {
    return (
      <View style={[styles.table, { borderColor: theme.cardBorder }]}>
        {/* Table header */}
        <View style={[styles.tableHead, { backgroundColor: theme.card, borderBottomColor: theme.cardBorder }]}>
          <Text style={[styles.thPos, { color: theme.textMuted }]}>POS</Text>
          <Text style={[styles.thDriver, { color: theme.textMuted }]}>DRIVER</Text>
          <Text style={[styles.thTeam, { color: theme.textMuted }]}>TIM</Text>
          {type === 'quali' ? (
            <Text style={[styles.thTime, { color: theme.textMuted }]}>WAKTU</Text>
          ) : (
            <>
              <Text style={[styles.thTime, { color: theme.textMuted }]}>WAKTU / STATUS</Text>
              <Text style={[styles.thPts, { color: theme.textMuted }]}>PTS</Text>
            </>
          )}
        </View>
        {/* Rows */}
        {results.map((result: any) => {
          ...
          const isTop = pos <= 3;
          return (
            <View key={result.position} style={[
              styles.tableRow,
              { borderBottomColor: theme.cardBorder },
              pos === 1 && { backgroundColor: theme.accentBg },
            ]}>
              <Text style={[styles.tdPos, { color: isTop ? theme.accent : theme.textMuted }]}>{pos}</Text>
              <View style={styles.tdDriver}>
                <View style={[styles.colorLine, { backgroundColor: color }]} />
                <FlagBadge code={driverCode} size="sm" />
                <View>
                  <Text style={[styles.tdName, { color: theme.text, fontWeight: theme.titleWeight }]}>
                    {result.Driver.givenName} {result.Driver.familyName}
                  </Text>
                  <Text style={[styles.tdCode, { color }]}>
                    {result.Driver.code ?? result.Driver.familyName.slice(0, 3).toUpperCase()}
                  </Text>
                </View>
              </View>
              <Text style={[styles.tdTeam, { color: theme.textMuted }]} numberOfLines={1}>{teamName}</Text>
              {type === 'quali' ? (
                <Text style={[styles.tdTime, { color: isTop ? theme.accent : theme.textSub }]}>
                  {result.Q3 ?? result.Q2 ?? result.Q1 ?? '-'}
                </Text>
              ) : (
                <>
                  <Text style={[styles.tdTime, { color: theme.textSub }]}>
                    {pos === 1 ? result.Time?.time : result.Time?.time ? `+${result.Time.time}` : result.status}
                  </Text>
                  <Text style={[styles.tdPts, { color: parseInt(result.points) > 0 ? theme.accent : theme.textMuted }]}>
                    {result.points}
                  </Text>
                </>
              )}
